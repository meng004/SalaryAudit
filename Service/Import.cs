...
namespace JournalVoucherAudit.Service
{
    public class Import
    {
        ...
        public Import(string filePath, int titleRowIndex, int lastRowIndex)
        {
            _FilePath = filePath;
            ...
            where CaiWu : CaiWuItem, new()
        {
            ...
            //在list中插入数据
            //从标题行至最后行
            for (int i = _TitleRowIndex + 1; i <= sheet.LastRowNum - _LastRowIndex; i++)
            {
                //取excel的行数据
                ...
                //新建财务
                var caiWu = new CaiWu();
                caiWu.CreditAmount = sheetRow.GetCell(cells["应发工资"]).NumericCellValue;
                caiWu.Remark = sheetRow.GetCell(cells["部门名称"]).StringCellValue;
                caiWu.VoucherDate = sheetRow.GetCell(cells["姓名"]).StringCellValue;
                caiWu.VoucherNumber = sheetRow.GetCell(cells["人员代码"]).StringCellValue;
                list.Add(caiWu);
            }
            ...
            where GuoKu : GuoKuItem, new()
        {
            ...
            for (int i = _TitleRowIndex + 1; i <= sheet.LastRowNum - _LastRowIndex; i++)
            {
                ...
                //新建财务
                var guoKu = new GuoKu();
                guoKu.Amount = sheetRow.GetCell(cells["应发工资"]).NumericCellValue;
                guoKu.RemarkReason = sheetRow.GetCell(cells["人员代码"]).StringCellValue;
                guoKu.CreateDate = sheetRow.GetCell(cells["姓名"]).StringCellValue;
                guoKu.PaymentNumber = sheetRow.GetCell(cells["部门名称"]).StringCellValue;
                list.Add(guoKu);
            }
